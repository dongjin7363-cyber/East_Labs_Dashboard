...
function normalizeSnapshotForDb(
  ...
  return {
    ...
    memo: snapshot.memo?.trim() ? snapshot.memo.trim() : null,
...
export class SupabaseTotalAssetRepository implements TotalAssetRepository {
  ...
  async getSnapshots(): Promise<TotalAssetSnapshot[]> {
    const { data, error } = await supabase
      ...
      .select("id,user_id,date,total_asset_krw_int,fx_rate,memo,updated_at")
  ...
  async upsertSnapshot(
    snapshot: TotalAssetSnapshot,
    _options?: UpsertTotalAssetOptions,
  ): Promise<void> {
    void _options;
    const payload = normalizeSnapshotForDb(snapshot, this.userId);
